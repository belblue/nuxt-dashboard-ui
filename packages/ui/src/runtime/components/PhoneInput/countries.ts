 ...
 * Each letter is mapped to a Regional Indicator Symbol (Unicode offset 127397).
 */
...
/**
 * Find a country by its dial code string (e.g., "+34").
 * For shared codes like "+1", prioritises US.
 */
export function findCountryByDialCode(
  dialCode: string
): Country | undefined {
  const normalized = dialCode.startsWith("+") ? dialCode : `+${dialCode}`;
  // Exact match — prioritise US for "+1"
  const usFirst = countries.find(
    (c) => c.dialCode === normalized && c.code === "US"
  );
...
/**
 * Full list of countries with calling codes, sorted alphabetically.
 */
